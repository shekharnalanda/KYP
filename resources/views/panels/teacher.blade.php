...
<div style="display:flex;justify-content:space-between;align-items:center;gap:18px;flex-wrap:wrap"><div><span class="eyebrow" style="color:#057d78;background:#e8fffd">TEACHER PANEL</span><h1>Teacher Dashboard</h1><p style="color:var(--muted)">Classroom sessions, learner progress और attendance overview।</p></div><div style="display:flex;gap:10px"><a class="btn btn-primary" href="{{ route('attendance.index') }}">Mark Attendance</a><form method="POST" action="{{ route('logout') }}">@csrf<button class="btn btn-light">Logout</button></form></div></div>
