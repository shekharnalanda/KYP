...
<form id="bulkDocs" method="POST" action="{{ route('admin.results.bulk') }}">@csrf</form>
<div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-top:20px">
<select form="bulkDocs" name="document_type" style="padding:10px;border-radius:10px"><option value="marksheet">Bulk Marksheets</option><option value="certificate">Bulk Certificates</option></select>
<button form="bulkDocs" class="btn btn-primary">Open Print/PDF View</button><span style="color:var(--muted)">Published records चुनें</span>
</div>
...
<td><input form="bulkDocs" class="result-check" type="checkbox" name="result_ids[]" value="{{ $result->id }}" @disabled(!$result->published_at)></td>
