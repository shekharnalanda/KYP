...
<div style="display:flex;justify-content:space-between;align-items:center;gap:18px;flex-wrap:wrap"><div><span class="eyebrow" style="color:#057d78;background:#e8fffd">ADMIN CONTROL</span><h1>KYP Administration</h1><p style="color:var(--muted)">Learning, assessment और certification का live central overview।</p></div><div style="display:flex;gap:10px"><a class="btn btn-primary" href="{{ route('attendance.index') }}">Attendance Operations</a><form method="POST" action="{{ route('logout') }}">@csrf<button class="btn btn-light">Logout</button></form></div></div>
...
<div class="panel-card"><h3>Online Exams</h3><div class="metric">{{ $stats['exams'] }}</div></div>
<div class="panel-card"><h3>Question Bank</h3><div class="metric">{{ $stats['questions'] }}</div></div>
<div class="panel-card"><h3>Results</h3><div class="metric">{{ $stats['results'] }}</div></div>
<div class="panel-card"><h3>Certificates</h3><div class="metric">{{ $stats['certificates'] }}</div></div>
</div></div></div></div>
@endsection
