...
<div style="display:flex;justify-content:space-between;align-items:center;gap:18px"><div><span class="eyebrow" style="color:#057d78;background:#e8fffd">STUDENT PANEL</span><h1>नमस्ते, {{ auth()->user()->name }}</h1><p style="color:var(--muted)">आपकी classroom तथा lab progress और exam eligibility।</p></div><form method="POST" action="{{ route('logout') }}">@csrf<button class="btn btn-light">Logout</button></form></div>
